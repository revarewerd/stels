...
import io.netty.handler.codec.{ByteToMessageDecoder, DelimiterBasedFrameDecoder, Delimiters, LengthFieldBasedFrameDecoder}
...
import ru.sosgps.wayrecall.utils.SimpleChannelInboundHandlerStateMachine
...
/**
  ...
  */
class TK102Server extends Netty4Server with grizzled.slf4j.Logging {

  override protected def initPipeLine(pipeline: ChannelPipeline): Unit = {
    pipeline.addLast(
      new LoggingHandler(classOf[TK102Server]),
      new TK102MessageDecoder(),
      new TK102rAvlDataDecoder(store)
    )
  }

}

class TK102MessageDecoder extends ByteToMessageDecoder  with grizzled.slf4j.Logging {
...
class TK102rAvlDataDecoder(store: PackProcessor) extends  SimpleChannelInboundHandler[ByteBuf] with grizzled.slf4j.Logging {
  ...
  override def channelRead0(ctx: ChannelHandlerContext, msg: ByteBuf): Unit = {
    ...
    command match {
      ...
      case "BR00" =>
        ...
        gpsdata.data.put("protocol", "TK102")

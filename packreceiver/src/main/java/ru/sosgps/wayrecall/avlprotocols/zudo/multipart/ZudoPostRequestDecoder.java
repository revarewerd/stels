 ...
 * under the License.
 */
package ru.sosgps.wayrecall.avlprotocols.zudo.multipart;

import io.netty.buffer.ByteBuf;
import io.netty.handler.codec.DecoderException;
import io.netty.handler.codec.http.*;
import ru.sosgps.wayrecall.avlprotocols.zudo.multipart.HttpPostBodyUtil.SeekAheadNoBackArrayException;
import ru.sosgps.wayrecall.avlprotocols.zudo.multipart.HttpPostBodyUtil.SeekAheadOptimize;
import ru.sosgps.wayrecall.avlprotocols.zudo.multipart.HttpPostBodyUtil.TransferEncodingMechanism;
 ...
 *
 */
public class ZudoPostRequestDecoder {
     ...
     *             errors
     */
    public ZudoPostRequestDecoder(HttpRequest request) throws ErrorDataDecoderException,
     ...
     *             errors
     */
    public ZudoPostRequestDecoder(HttpDataFactory factory, HttpRequest request) throws ErrorDataDecoderException,
            IncompatibleDataDecoderException {
     ...
     *             errors
     */
    public ZudoPostRequestDecoder(HttpDataFactory factory, HttpRequest request, Charset charset)
    ...
    private void checkDestroyed() {
        if (destroyed) {
            throw new IllegalStateException(ZudoPostRequestDecoder.class.getSimpleName() + " was destroyed already");
        }
    }
     ...
     *             errors
     */
    public ZudoPostRequestDecoder offer(HttpContent content) throws ErrorDataDecoderException {
        checkDestroyed();
            ...
            loop: while (sao.pos < sao.limit) {
                ...
                case DISPOSITION:// search '='
                    if (read == '=') {
                        currentStatus = MultiPartStatus.FIELD;
                        equalpos = currentpos - 1;
                        key = decodeAttribute(undecodedChunk.toString(firstpos, equalpos - firstpos, charset),
                                charset);
                        currentAttribute = factory.createAttribute(request, key);
                        firstpos = currentpos;
                    } else if (read == '&') { // special empty FIELD
    ...
    private void setFinalBuffer(ByteBuf buffer) throws ErrorDataDecoderException, IOException {
        ...
        try {
            ...
            if (!currentAttribute.getName().equals("bin"))
                currentAttribute.setValue(value);
        } catch (ErrorDataDecoderException e) {
            //currentAttribute.setValue(null);
        }
        addHttpData(currentAttribute);
        currentAttribute = null;
    }
    ...
    /**
     * Destroy the {@link ZudoPostRequestDecoder} and release all it resources. After this method

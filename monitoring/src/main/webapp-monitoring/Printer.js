...
Ext.define("Ext.ux.grid.Printer", {
    ...
    requires: ['Ext.XTemplate', 'Ext.Date'],
...
//            });
            ...
            if (this.stylesheetPath === null) {
                ...
                this.stylesheetPath = scriptPath.substring(0, scriptPath.indexOf('Printer.js')) + 'print.css';
            }
            ...
            if (expanderTemplate) {
                pluginsBodyMarkup = [
                    '<tr class="{[xindex % 2 === 0 ? "even" : "odd"]} expanded-row"><td colspan="' + columns.length + '">',
            ...
            var htmlMarkup = [
                ...
                '</html>',
                {
                    ...
                    renderColumn: function(column, value, rcd, col)
                    {
                        ...
                        if(column.xtype == 'datecolumn' && column.format)
                            value = Ext.Date.format( new Date(value), column.format );
                        else if (column.xtype == 'templatecolumn')
                    	{
...
//        },
         ...
         * @type String
         */
        printLinkText: 'Печать документа',
        ...
        headerTpl: [ 
            '<tpl for=".">',
                '<tpl if="initialConfig.width">',
                    '<th width="{initialConfig.width}" style="text-align: {align}">{text}</th>',
                '<tpl else>',

Ext.define('Seniel.view.reports.MovementStats', {
    extend: 'Ext.grid.Panel',
    title: tr('main.reports.statistics'),
    initComponent: function () {
        Ext.apply(this, {
            store: Ext.create('EDS.store.MovementStats', {
                autoSync: false
            })
        });
...
//        },
        ...
        },
        {
            ...
            handler: function(btn) {
                ...
                var reportsList = [];
                reportsList.push({label: tr('main.reports.map'), input: 'map'});
                    ...
                    for (var i = 0; i < tbitems.length; i++) {
                        if (tbitems[i].repCodeName) {
                            ...
                            if (tbitems[i].repCodeName === 'sensor') {
                                params.sensor = (tbitems[i].curSensors)?(tbitems[i].curSensors):('');
                            }
                        }
                    }
                } else {
        ...
        //         var params = grid.getStore().getProxy().extraParams;
        //
        //         var wnd = Ext.create('Seniel.view.reports.MapGeneratorWindow', {
        //             reportParams: {
        //                 uid: params.selected,
        //                 from: params.from,
        //                 to: params.to
    ...
    refreshReport: function (settings) {
        ...
        store.on('load', function(store) {
            var tabs = self.up('reportwnd').down('tabpanel').getTabBar().items;
            var isEmpty = (store.getAt(3).get('value') > 0)?false:true;

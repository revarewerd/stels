...
Ext.define('Seniel.view.reports.GroupReportWindow', {
    ...
    requires: [
        ...
        'Seniel.view.reports.GroupReportMap',
        'Seniel.view.DygraphPanel',
        'Seniel.view.reports.GroupMovementStats',
        'Seniel.view.WRWindow'
    ],
    alias: 'widget.groupreportwnd',
    stateId: 'repWnd',
    stateful: true,
    minWidth: 640,
    minHeight: 480,
                    ...
                    items: [
                        {
                            ...
                            fieldLabel: tr('basereport.group'),
                            ...
                            anyMatch: true,
                            caseSensitive: false,
                            store:Ext.create('EDS.store.UserGroupsOfObjects')
                            //{
                            //    fields: ['uid', 'name'],
                            //    data: {
                            //        'items': []
                            ...
                            items: [
                                {
                                    ...
                                    handler: function () {
                                        var d = new Date();
                                        this.up('groupreportwnd').down('datefield[itemId="fromDate"]').setValue(d);
                                        this.up('groupreportwnd').down('datefield[itemId="toDate"]').setValue(d);
                                    }
                                },
                                {
                                    xtype: 'button',
                        ...
                        },
                        {
                            ...
                            handler: function () {
                                var groupreportwnd = this.up('groupreportwnd'),
                                    selectedObject = groupreportwnd.getSelected(),
                                    dates = groupreportwnd.getWorkingDates();
...
//                                    console.log('Кнопка окна=', win.ctrlbtn);
                                    ...
                                    var settings = {
                                        dates: groupreportwnd.getWorkingDates(),
                                        selected: groupreportwnd.getSelected()
                                    };
...
//                                    }
                                    ...
                                    if(groupreportwnd.tabShown)
                                    {
                                        for(var k in groupreportwnd.tabShown)
                                            groupreportwnd.tabShown[k] = false;
                                        if(rdItems.length > 0)
                                        {
                                            groupreportwnd.postStats(rdItems[0].itemId);
                                            groupreportwnd.tabShown[rdItems[0].itemId] = true;
                                        }
    ...
    initComponent: function() {
        ...
        cb.getStore().on("load", function (store) {
            cb.fireEvent('storeload',store.getAt(0))
         })
        cb.getStore().load();
        cb.on('storeload',function(firstitem){
            cb.select(firstitem)
        })
        ...
        this.on('boxready', function(wnd) {
            ...
            //    cb = wnd.down('combobox'),
                rm = wnd.down('groupreportmap');
            //cb.getStore().add(viewport.down('mapobjectlist').roData);
            //cb.selectcb.getStore().getAt(0);
            ...
            if (widgets.length > 1) {
                var repdataPanel=mainPanel.add(Ext.create('Ext.tab.Panel', {
                    flex: 1,
                    floatable: true,
                ...
                repdataPanel.on("tabchange",function( tabPanel, newCard, oldCard, eOpts ){
                    if(oldCard.removeSelection) {oldCard.removeSelection()}
                })
            } else {

...
public class EquipmentTypesCreateEvent implements Serializable,WayrecallAxonEvent {
    @Override
    public Option<String> getInitialName() {
        List<String> fields = JavaConversions.seqAsJavaList(EventsViewConverter.eqTypeNameFields());
        StringBuilder result = new StringBuilder("");
        String sep = "";
        for(String k : fields) {
            if(data.get(k) != null) {
                result.append(sep);
                result.append(data.get(k).toString());
                sep = " ";
            }
        }
        return Option.apply(result.toString());
    }
    ...
    public EquipmentTypesCreateEvent(ObjectId eqTypeId,HashMap<String, Object> data) {
